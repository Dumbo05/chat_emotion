...
for i in range(1,10):
 for j in range(1,10-i):
  k=10-i-j
  if k>0:candidates.append((('l7','l57b','l579w'),[i/10,j/10,k/10]))
records=[]
...
best=max(records,key=lambda r:(r['val_macro_f1'],r['val_accuracy']));summary={'protocol_name':'wavlm_clean_v3','used_test_set':False,'test_status':split['test_status'],'member_definitions':members,'candidate_count':len(records),'best':best,'created_at':datetime.now(timezone.utc).isoformat()};(ROOT/'models/speech/wavlm_clean_v3_ensemble_refined_summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf8');print(json.dumps({k:best[k] for k in ('run_id','ensemble_members','ensemble_weights','val_accuracy','val_macro_f1')},ensure_ascii=False,indent=2))
