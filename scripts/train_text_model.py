...
class EmotionDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        encoded = self.tokenizer(
            str(row.text), truncation=True, max_length=self.max_length, padding=False
        )
...
def predict(model, loader, device) -> tuple[list[int], list[int], float]:
    model.eval()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fine-tune mBERT or XLM-R on fixed CSV splits.")
    ...
    frames = {
        split: pd.read_csv(args.data_dir / f"{split}.csv")
    ...
    update_steps = max(
        1, (len(loaders["train"]) * args.epochs) // args.gradient_accumulation
    )
    scheduler = get_linear_schedule_with_warmup(
        optimizer, num_warmup_steps=int(update_steps * 0.1), num_training_steps=update_steps
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_pred, val_true, val_loss = predict(model, loaders["validation"], device)
    ...
    test_pred, test_true, test_loss = predict(best_model, loaders["test"], device)
    ...
    pd.DataFrame(confusion, index=EMOTIONS, columns=EMOTIONS).to_csv(
    ...
    for language in sorted(test_frame["language"].unique()):
        ...
        report[f"test_{language}"] = metrics(
            [test_pred[index] for index in indices], [test_true[index] for index in indices]
        )
